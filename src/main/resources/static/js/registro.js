var selectedType = 'usuario';
        ...
        function goStep(n) {
            ...
            allSteps.forEach(function(id){ document.getElementById(id).style.display = 'none'; });
        ...
        function handleFotoPerfilChange(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    document.getElementById('fotoPerfilText').textContent = input.files[0].name;
        ...
        function handleDocumentosChange(input) {
            if (input.files && input.files.length > 0) {
                ...
                document.getElementById('docIcon').innerHTML = '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" stroke="#10b981"/><polyline points="22 4 12 14.01 9 11.01" stroke="#10b981"/>';
                document.getElementById('documentosText').textContent = input.files.length + ' archivo(s) seleccionado(s)';
                document.getElementById('documentosText').style.color = '#10b981';
                document.getElementById('documentosSubtext').textContent = names.length > 50 ? names.substring(0,50)+'...' : names;
